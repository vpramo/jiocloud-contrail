...
Puppet::Type.type(:contrail_analytics).provide(
  :provisioner,
  :parent => Puppet::Provider::ContrailBGP
) do

  commands  :provision_analytics => '/usr/sbin/contrail-provision-analytics'

  def getUrl
    'http://' + resource[:api_server_address] + ':' + resource[:api_server_port] + '/analytics'
  end
  ...
  def host_address=(value)
    fail('Cannot change Existing value, please remove and recreate the Analytics node object (' + resource[:name] + ')')
  end
